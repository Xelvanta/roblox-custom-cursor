# -*- coding: utf-8 -*-
"""Main launcher for Roblox Custom Cursor application."""
...
def get_config_path():
    ...
    config_folder = os.path.join(config_dir, "Xelvanta Softworks", "Roblox Custom Cursor")
...
class CursorViewerApp(tk.Tk):
    ...
    def load_config(self):
        try:
            with open(CONFIG_PATH, "r", encoding="utf-8") as f:
                # Uncomment the following line to debug config path
                # print(CONFIG_PATH)
                return json.load(f)
        except Exception:
    ...
    def __init__(self, image_data):
        ...
        self.icon_image = tk.PhotoImage(file=abs_icon_path)
    ...
    def build_ui(self, image_data):
        ...
        credits_text = "Made with ♡ by Alina"
    ...
    def add_buttons(self, container, col, filepath, label_text, pil_image):
        ...
        change_button = tk.Button(container, text="Change Cursor", command=change_button_action,
        ...
        def shrink_image():
            try:
                ...
                # Shrink by 2px (to 62x62), then paste centered on transparent 64x64
                new_img = Image.new("RGBA", (64, 64), (0, 0, 0, 0))
                shrunk = img.resize((62, 62), Image.Resampling.LANCZOS)
                new_img.paste(shrunk, (1, 1))
                new_img.save(filepath, format="PNG")
                self.update_gui_with_new_image(filepath, label_text)
            except Exception as e:
                messagebox.showerror("Error", f"Error shrinking image:\n\n{e}")

        def enlarge_image():
            try:
                img = Image.open(filepath).convert("RGBA")
                # Enlarge by 2px (to 66x66), then crop center 64x64
                enlarged = img.resize((66, 66), Image.Resampling.LANCZOS)
                left = (66 - 64) // 2
                top = (66 - 64) // 2
                cropped = enlarged.crop((left, top, left + 64, top + 64))
                cropped.save(filepath, format="PNG")
                self.update_gui_with_new_image(filepath, label_text)
            except Exception as e:
                messagebox.showerror("Error", f"Error enlarging image:\n\n{e}")

        # --- Nudge Arrows or Photopea Button, depending on config ---
        if self.config.get("nudge_mode", True):
            ...
            shrink_btn = tk.Button(
                nudge_frame, text="−", width=3, command=shrink_image,
                bg="#444444", fg="white", cursor="hand2"
            )
            enlarge_btn = tk.Button(
                nudge_frame, text="+", width=3, command=enlarge_image,
                bg="#444444", fg="white", cursor="hand2"
            )
            shrink_btn.grid(row=1, column=0)
            enlarge_btn.grid(row=1, column=2)
            up_btn.grid(row=1, column=1)
            left_btn.grid(row=2, column=0)
            down_btn.grid(row=2, column=1)
            right_btn.grid(row=2, column=2)

            for btn in (up_btn, left_btn, down_btn, right_btn, shrink_btn, enlarge_btn):
            ...
            photopea_button = tk.Button(container, text="Edit in Photopea", command=photopea_button_action,
                                        bg="#444444", fg="white", width=13, cursor="hand2")
    ...
    def show_settings_window(self):
        ...
        create_button_with_info(settings_win, "Export Cursors as Profile", self.export_cursors_to_rcur,
                                "Export your currently applied cursors as a Roblox Custom Cursor Profile (.rcur) file.\n"
                                "This file can be shared or imported later to restore your full cursor set.")

        create_button_with_info(settings_win, "Import Cursors from Profile", self.import_cursors_from_rcur,
                                "Import cursors from an existing Roblox Custom Cursor Profile (.rcur) file.\n"
                                "This will replace your currently applied cursor set with the full set from the profile.")

        def get_nudge_mode_text():
            return (
                "Switch to Photopea Buttons"
                if self.config.get("nudge_mode", True)
                else "Switch to Nudge Arrows"
            )

        def on_toggle_nudge_mode():
            self.config["nudge_mode"] = not self.config.get("nudge_mode", True)
            self.save_config()
            nudge_btn.config(text=get_nudge_mode_text())
        ...
        nudge_btn, nudge_info = create_button_with_info(
            settings_win,
            get_nudge_mode_text(),
    ...
    def start_refresh_listener(self):
        ...
        def listener():
            try:
                ...
                # Port already in use — skip listener
                pass
